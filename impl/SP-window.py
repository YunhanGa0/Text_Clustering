...
class SinglePassClustering:
    def __init__(self, threshold=0.5, time_window=24):
        """
        初始化单程聚类算法，带有时间窗口

        Args:
            threshold: 相似度阈值，大于此阈值会被归为同一簇
            time_window: 时间窗口大小，单位为小时，文档必须在同一窗口内才能被聚到一起
        """
        self.threshold = threshold
        self.time_window = time_window * 3600  # 转换为秒
        self.clusters = []  # 每个簇保存的是向量的平均表示
        self.cluster_times = []  # 保存每个簇的平均时间戳
        self.cluster_contents = []  # 保存每个簇的原始文本索引

    def fit(self, vectors, timestamps=None):
        """
        对文本向量进行聚类

        Args:
            vectors: 文本向量矩阵
            timestamps: 每个文本对应的时间戳列表，如果为None则不考虑时间因素

        Returns:
            cluster_contents: 聚类结果，每个簇中包含的文档索引
        """
        is_sparse = hasattr(vectors, "toarray")
        use_time = timestamps is not None
        ...
        for idx, vec in enumerate(vectors):
            ...
            current_time = timestamps[idx] if use_time else None

            if not self.clusters:
                self.clusters.append(vec)
                self.cluster_contents.append([idx])
                if use_time:
                    self.cluster_times.append(current_time)
                continue
            ...
            for c_idx, c_vec in enumerate(self.clusters):
                # 如果需要考虑时间窗口
                if use_time and current_time is not None and self.cluster_times[c_idx] is not None:
                    time_diff = abs(current_time - self.cluster_times[c_idx])
                    # 如果时间差超出窗口，跳过这个簇
                    if time_diff > self.time_window:
                        continue

                # 计算文本相似度
                sim = cosine_similarity(vec, c_vec)[0][0]
                valid_clusters.append(c_vec)
            ...
            max_pos = valid_similarities.index(max_sim)
            max_index = valid_indices[max_pos]
            ...
            if max_sim >= self.threshold:
                ...
                # 更新该簇的平均时间戳
                if use_time:
                    cluster_times = [timestamps[i] for i in cluster_indices]
                    valid_times = [t for t in cluster_times if t is not None]
                    if valid_times:
                        self.cluster_times[max_index] = sum(valid_times) / len(valid_times)
            else:
                self.clusters.append(vec)
                self.cluster_contents.append([idx])
                if use_time:
...
def generate_html_report(clusters, contents, time_data, file_path, threshold, time_window=24):
    """
    生成HTML聚类报告，按时间窗口分组展示
    """
    html = f"""
    <!DOCTYPE html>
    <html>
    ...
    <body>
        ...
        <div class="summary">
            ...
            <p><strong>时间窗口大小:</strong> {time_window} 小时</p>
            <p><strong>总文档数:</strong> {len(contents)}</p>
            <p><strong>总簇数:</strong> {len(clusters)}</p>
            <p><strong>最大簇大小:</strong> {max([len(c) for c in clusters]) if clusters else 0}</p>
            <p><strong>最小簇大小:</strong> {min([len(c) for c in clusters]) if clusters else 0}</p>
            <p><strong>平均簇大小:</strong> {sum([len(c) for c in clusters]) / len(clusters) if clusters else 0:.2f}</p>
        </div>
    """

    # 如果没有时间数据，按照原有方式展示
    if not time_data:
        html += "<h2>聚类结果 (无时间窗口信息)</h2>"
        for idx, cluster in enumerate(clusters):
            html += generate_cluster_html(idx, cluster, contents, None)
    ...
    for cluster in clusters:
        ...
        if cluster_times:
            # 确保所有时间都转换为时间戳（浮点数表示的秒数）
            timestamp_values = []
            for t in cluster_times:
                if isinstance(t, datetime.datetime):
                    timestamp_values.append(t.timestamp())
                elif isinstance(t, (int, float)):
                    # 假设已经是时间戳
                    timestamp_values.append(float(t))
                elif isinstance(t, str):
                    try:
                        dt = parse(t)
                        timestamp_values.append(dt.timestamp())
                    except:
                        continue
                else:
                    continue
            ...
            # 计算平均时间戳
            if timestamp_values:
                avg_time = sum(timestamp_values) / len(timestamp_values)
                cluster_avg_times.append(avg_time)
            else:
                cluster_avg_times.append(None)
        else:
            cluster_avg_times.append(None)

    # 找出所有有效时间戳的最小和最大值
    valid_timestamps = [t for t in cluster_avg_times if t is not None]
    if not valid_timestamps:
        html += "<h2>聚类结果 (无有效时间信息)</h2>"
        for idx, cluster in enumerate(clusters):
            html += generate_cluster_html(idx, cluster, contents, time_data)
    ...
    # 根据时间窗口大小划分时间段
    time_window_seconds = time_window * 3600  # 转换为秒
    ...
    while current_start <= max_time:
        current_end = current_start + time_window_seconds
    ...
    # 将簇分配到相应的时间窗口
    for idx, (cluster, avg_time) in enumerate(zip(clusters, cluster_avg_times)):
        if avg_time is None:
            continue

        # 找到对应的时间窗口
        for (start_time, end_time), window_data in time_windows.items():
            if start_time <= avg_time < end_time:
                window_data['clusters'].append((idx, cluster))
                break

    # 按时间窗口生成HTML报告
    for (start_time, end_time), window_data in sorted(time_windows.items()):
        if not window_data['clusters']:
            continue

        start_time_str = datetime.datetime.fromtimestamp(start_time).strftime('%Y-%m-%d %H:%M')
        end_time_str = datetime.datetime.fromtimestamp(end_time).strftime('%Y-%m-%d %H:%M')

        html += f"""
        <div class="time-window">
            <div class="time-window-header">
                <h2>时间窗口 {window_data['index'] + 1}: {start_time_str} 至 {end_time_str} (共 {len(window_data['clusters'])} 个簇)</h2>
            </div>
        """

        # 添加该窗口下的所有簇
        for cluster_idx, cluster in window_data['clusters']:
            html += generate_cluster_html(cluster_idx, cluster, contents, time_data)

        html += "</div>"

    # 添加没有时间信息的簇
    no_time_clusters = [(idx, cluster) for idx, (cluster, avg_time) in enumerate(zip(clusters, cluster_avg_times)) if avg_time is None]
    if no_time_clusters:
        html += f"""
        <div class="time-window">
            <div class="time-window-header">
                <h2>未知时间窗口 (共 {len(no_time_clusters)} 个簇)</h2>
            </div>
        """

        for cluster_idx, cluster in no_time_clusters:
            html += generate_cluster_html(cluster_idx, cluster, contents, time_data)
...
def generate_cluster_html(idx, cluster, contents, time_data):
    """生成单个簇的HTML内容"""
    html = f"""
    <div class="cluster">
        <div class="cluster-header">
            <h3>簇 #{idx + 1} (包含 {len(cluster)} 条文档)</h3>
        </div>
    """
...
def cluster_texts(file_path, content_field='content', time_field=None, threshold=0.5,
                  time_window=24, output_dir="output", max_docs=1000, use_transformer=True):
    """
    ...
    if time_data:
        ...
        if any(t is None for t in timestamps):
            print(f"警告: 时间数据中存在缺失值，将使用平均时间戳填充")
            valid_times = [t for t in timestamps if t is not None]
            avg_time = sum(valid_times) / len(valid_times) if valid_times else 0
            timestamps = [t if t is not None else avg_time for t in timestamps]
    ...
    # 聚类
    print(f"开始使用阈值 {threshold} 和时间窗口 {time_window}小时 进行单程聚类...")
    model = SinglePassClustering(threshold=threshold, time_window=time_window)
    clusters = model.fit(vectors, timestamps)
    ...
    generate_html_report(clusters, contents, time_data, html_path, threshold, time_window)
...
if __name__ == '__main__':
    ...
    TIME_FIELD = None        # 自动检测时间字段
    TIME_WINDOW = 8         # 时间窗口，单位为小时
